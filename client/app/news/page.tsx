...
import { getNewsOptimized, getFeaturedNewsOptimized, getGlobalCached } from '@/lib/strapi-optimized';
...
export async function generateMetadata({ searchParams }: NewsPageProps): Promise<Metadata> {
  try {
    ...
    let title = 'الأخبار';
    let description = 'تصفح جميع أخبار شروع والتحديثات';

    if (page && parseInt(page) > 1) {
      title = `الأخبار - الصفحة ${page}`;
      description = `أخبار شروع - الصفحة ${page}`;
    }
    ...
    return {
      title: 'الأخبار | شروع',
      description: 'تصفح جميع أخبار شروع والتحديثات',
    };
  }
}
...
export default async function NewsPage({ searchParams }: NewsPageProps) {
  try {
    // Fetch news and featured news using optimized functions
    const [allNewsResponse, featuredNewsResponse] = await Promise.all([
      getNewsOptimized(),
      getFeaturedNewsOptimized(3)
    ]);
        ...
        <section className="bg-black text-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: allNews[0].description }}
                    />
                  </div>
                  ...
                  <div className="border-t border-gray-800 pt-6">
                    ...
                    <p className="text-gray-400 mb-4 font-['IBM_Plex_Sans_Arabic']">
                      {formatDate(allNews[0].news_date)}
                    </p>

                    {/* Action Buttons */}
                    <div className="flex flex-col sm:flex-row gap-4">
                      <Link
                        href={`/news/${allNews[0].slug}`}
                      ...
                      {allNews[0].source_url && (
                        <a
                          href={allNews[0].source_url}
                          target="_blank"
              ...
              {/* Hero Image */}
              {allNews.length > 0 && allNews[0].cover_image && (
                <div className="relative aspect-[4/3] overflow-hidden shadow-2xl">
                  <Image
                    src={getStrapiMedia(allNews[0].cover_image.url) || '/placeholder.jpg'}
                    alt={allNews[0].cover_image.alternativeText || allNews[0].title}
                    fill
                  ...
                  >
                    {news.cover_image && (
                      <div className="relative aspect-[16/9] overflow-hidden">
                        <Image
                          src={getStrapiMedia(news.cover_image.url) || '/placeholder.jpg'}
                    ...
                    <div className="p-6">
                      ...
                      {news.source && (
                        <p className="text-gray-500 text-sm font-['IBM_Plex_Sans_Arabic']">
                          المصدر: {news.source}
                        </p>
                      )}
                    </div>
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-['IBM_Plex_Sans_Arabic']">
            لم نتمكن من تحميل صفحة الأخبار. يرجى المحاولة مرة أخرى لاحقاً.
          </p>
        </div>
      </div>
    );
